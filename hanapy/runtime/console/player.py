...
class ConsolePlayerActor(PlayerActor):
    def get_next_action(self, view: PlayerView) -> Action:
        ...
        print("-" * 10)
        action = self.parse_action_from_input(view.me, view.state.config.max_cards, view.cards)
        print("-" * 10)
        return action
    ...
    def parse_action_from_input(self, me: int, max_cards: int, cards: List[List[Card]]) -> Action:
        while True:
            msg = input()
            try:
